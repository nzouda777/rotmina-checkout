...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (paymentMethod === 'card') {
      ...
      // Use the thtk pre-generated by the client (from /api/checkout/handshake) when
      // available — this ensures the same token used in TzlaHostedFields.create() is
      // also used in charge(), which Tranzila requires to validate the handshake.
      let thtk = providedThtk || null
      if (!thtk) {
        thtk = await tranzila.getHandshakeToken(chargeAmount, currencyCode)
      }
      console.log(`[CHARGE][${logId}] thtk source: ${providedThtk ? 'client-preloaded' : 'server-generated'}`)
