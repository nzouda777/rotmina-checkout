...
const STATUS_META: Record<string, { label: string; color: string; dot: string }> = {
  paid:        { label: 'Paid',        color: 'text-green-700 bg-green-50 border-green-100',  dot: 'bg-green-500' },
  failed:      { label: 'Failed',      color: 'text-red-700 bg-red-50 border-red-100',        dot: 'bg-red-500' },
  pending:     { label: 'Pending',     color: 'text-amber-700 bg-amber-50 border-amber-100',  dot: 'bg-amber-400' },
  processing:  { label: 'Processing',  color: 'text-blue-700 bg-blue-50 border-blue-100',     dot: 'bg-blue-500' },
  pending_3ds: { label: '3DS',         color: 'text-purple-700 bg-purple-50 border-purple-100', dot: 'bg-purple-500' },
  pending_bit: { label: 'Bit',         color: 'text-orange-700 bg-orange-50 border-orange-100', dot: 'bg-orange-400' },
  expired:     { label: 'Expired',     color: 'text-gray-500 bg-gray-50 border-gray-100',    dot: 'bg-gray-400' },
}
  ...
  if (error) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={load} className="px-4 py-2 rounded-lg bg-black text-white text-sm">Retry</button>
      </div>
    )
  }
    ...
    <div className="px-6 py-8 max-w-7xl mx-auto space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          <RevenueCard label="Today" value={fmt(data.revenue.today)} icon={<TrendingUp className="h-4 w-4" />} />
          <RevenueCard label="This Week" value={fmt(data.revenue.thisWeek)} icon={<TrendingUp className="h-4 w-4" />} accent />
          <RevenueCard label="This Month" value={fmt(data.revenue.thisMonth)} icon={<TrendingUp className="h-4 w-4" />} />
          <RevenueCard label="All Time" value={fmt(data.revenue.allTime)} icon={<TrendingUp className="h-4 w-4" />} />
        </div>
      </div>

      {/* Order stats */}
      <div>
        <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Orders</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <OrderCard
            label="Paid" value={data.orders.paid}
            icon={<CheckCircle2 className="h-5 w-5 text-green-600" />}
            bg="bg-green-50" border="border-green-100"
          />
          <OrderCard
            label="Failed" value={data.orders.failed}
            icon={<XCircle className="h-5 w-5 text-red-500" />}
            bg="bg-red-50" border="border-red-100"
          />
          <OrderCard
            label="In Progress" value={data.orders.active}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-sm text-gray-900">Recent Paid Orders</span>
            </div>
            <Link
              href="/admin/orders"
          ...
          {data.recentOrders.length === 0 ? (
            <div className="py-12 text-center text-sm text-gray-400">No paid orders yet</div>
          ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
            ...
            <p className="text-xs text-gray-400 mt-1">across {data.orders.paid} paid orders</p>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-semibold text-gray-900">
                  {fmt(data.giftCards.totalAmountIssued)}
                </span>
              </div>
            </div>
          </div>
